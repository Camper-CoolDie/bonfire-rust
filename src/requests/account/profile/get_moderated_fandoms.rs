...
#[derive(Serialize)]
pub(crate) struct GetModeratedFandomsRequest {
    #[serde(rename = "accountId")]
    id: u64,
    offset: u64,
}
impl GetModeratedFandomsRequest {
    pub(crate) fn new(id: u64, offset: u64) -> Self {
        Self { id, offset }
    }
}

impl Request for GetModeratedFandomsRequest {
    type Response = Response;
    type Error = InfallibleRequest<RootError>;

    async fn send_request(&self, client: &Client) -> Result<Response> {
        client
            .send_request("RFandomsGetAllModerated", self, Vec::new())
            .await
    }
}
