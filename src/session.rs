...
use hyper::{Request, Method, StatusCode, header};
...
impl Session {
		...
		String: From<T>
	{
		...
		let request = Request::builder()
			...
			.map_err(|err| Error::RequestBuilder(err))?;

		let mut response = self.sender.send_request(request).await
			.map_err(|err| Error::RequestSend(err))?;

		let status = response.status();
		if status.is_success() {
			let mut string = String::new();
			while let Some(next) = response.frame().await {
				let frame = next.map_err(|err| Error::ResponseReceive(err))?;
				if let Some(chunk) = frame.data_ref() {
					string.push_str(std::str::from_utf8(chunk)
						.map_err(|err| Error::ResponseUtf8(err))?);
				}
			}

			let body = json::parse(&string).map_err(|err| Error::ResponseParseJson(err))?;
			Ok(body)
		} else {
...
pub trait Connector {
	#[doc(hidden)]
	fn connect(&self) -> impl std::future::Future<Output = Result<SendRequest<Full<Bytes>>>> + Send;
...
impl Connector for InsecureConnector {
	async fn connect(&self) -> Result<SendRequest<Full<Bytes>>> {
		...
		let (sender, conn) = handshake(io).await.map_err(|err| Error::Handshake(err))?;
...
impl Connector for SecureConnector {
	async fn connect(&self) -> Result<SendRequest<Full<Bytes>>> {
		let stream = TcpStream::connect(self.addr.clone()).await
			.map_err(|err| Error::Connect(err))?;
		let connector = native_tls::TlsConnector::new().map_err(|err| Error::TlsConnector(err))?;
		let connector = TlsConnector::from(connector);
		let stream = connector.connect(&self.host.clone(), stream).await
			.map_err(|err| Error::TlsHandshake(err))?;
