...
fn minify_graphql() -> Result<(), Box<dyn Error>> {
        ...
        })
    {
        ...
        let query = fs::read_to_string(source).map_err(|error| {
            format!("failed to read GraphQL file {}: {error}", source.display())
        })?;
        let minified = minify(query).map_err(|error| {
            format!(
                "failed to minify GraphQL file {error:?}: {}",
                source.display()
            )
        })?;
        fs::write(&out, minified).map_err(|error| {
            format!(
                "failed to write minified GraphQL to {}: {error}",
                out.display()
            )
        })?;
    }
