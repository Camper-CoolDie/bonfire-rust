...
pub(crate) fn setup_single(fixture_path: &'static str) -> (Mock<'static>, Client) {
...
// FIXME: remove attribute when function is needed again
#[cfg(feature = "serde")]
pub(crate) fn setup_none() -> (Mock<'static>, Client) {
    let (client, endpoint) = setup();
    let mock = MOCK_SERVER.mock(|when, then| {
        when.path(endpoint);
        then.status(503);
